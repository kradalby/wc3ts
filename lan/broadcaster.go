...
func (b *Broadcaster) sendRawGameInfo(g *game.Game) {
	...
	_, err = b.conn.WriteTo(data, b.localhostAddr)
	if err != nil {
		slog.Debug("failed to send game to localhost", "game", g.Info.GameName, "error", err)
	}
}
...
func (b *Broadcaster) sendRefreshGame(hostCounter, slotsUsed, slotsAvailable uint32) {
	...
	_, err = b.conn.WriteTo(packet, b.localhostAddr)
	if err != nil {
		slog.Debug("failed to send refresh to localhost", "error", err)
	}
}
...
func (b *Broadcaster) sendDecreateGame(hostCounter uint32) {
	...
	_, err = b.conn.WriteTo(packet, b.localhostAddr)
	if err != nil {
		slog.Debug("failed to send decreate to localhost", "error", err)
	}
}
